...
type MyCercle = { id: string; nom: string; grade: string; tile: readonly string[]; membres: string; mode: string; unread: number; xp: number; xpMax: number; jour: number };
...
const GRADES: Record<string, Grade> = {
    graine: { label: 'Graine', bg: 'rgba(123,224,160,0.16)', border: 'rgba(123,224,160,0.35)', color: '#36b46a', icon: 'leaf', iconColor: '#2bb36b' },
    etoile: { label: 'Étoile', bg: 'rgba(255,210,122,0.16)', border: 'rgba(255,210,122,0.35)', color: '#e0a93a', icon: 'star', iconColor: '#ffb300' },
    flamme: { label: 'Flamme', bg: 'rgba(255,157,107,0.16)', border: 'rgba(255,157,107,0.35)', color: '#ff9d5c', icon: 'flame', iconColor: '#ff7a3c' },
};
...
export default function Cercles() {
    ...
    const created = useStore((st) => st.created);
    const isDec = view === 'decouvrir';

    const setSeg = (v: 'decouvrir' | 'mes') => { animateLayout(); setView(v); };

    const num = (str: string) => parseFloat(str.replace(',', '.')) || 0;
    const nearShown = NEARBY.filter((n) => {
        if (query.trim() && !n.nom.toLowerCase().includes(query.trim().toLowerCase())) return false;
        if (filter === 1) return num(n.dist) <= 3;
        if (filter === 2) return n.dot;
        if (filter === 3) return num(n.info) >= 8;
        return true;
    });

    const createdAsMine: MyCercle[] = created.map((cr) => {
        const t = themeOf(cr.theme);
        return { id: cr.id, nom: cr.nom, grade: 'graine', tile: [t.from, t.to, '#3a1c5e'], membres: `${cr.membres}/${cr.max}`, mode: cr.type === 'rencontre' ? 'Rencontre' : 'Amitié', unread: 0, xp: 20, xpMax: 500, jour: 1 };
    });
    const allMine: MyCercle[] = [...createdAsMine, ...(MINE as unknown as MyCercle[])];
    ...
    return (
        <Screen padded={false}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: PAD, paddingBottom: 140 }}>
                <FadeInUp delay={50}>
                    <View style={s.search}>
                        <Ionicons name="search" size={19} color={c.ink(0.45)} />
                        <TextInput
                            value={query} onChangeText={setQuery}
                            placeholder="Rechercher un cercle, une passion…" placeholderTextColor={c.ink(0.4)}
                            style={s.searchInput} returnKeyType="search"
                        />
                        {query.length > 0 && (
                            <Pressable onPress={() => setQuery('')} hitSlop={8}><Ionicons name="close-circle" size={18} color={c.ink(0.35)} /></Pressable>
                        )}
                    </View>
                </FadeInUp>
                ...
                <FadeInUp delay={90}>
                    <View style={s.segment}>
                        ...
                        <Seg label="Mes cercles" count={String(2 + created.length)} active={!isDec} onPress={() => setSeg('mes')} />
                    </View>
                </FadeInUp>
                ...
                {isDec ? (
                    <View>
                            ...
                            <View style={s.featured}>
                                ...
                                <View style={{ padding: 16 }}>
                                    ...
                                    <View style={s.reasonWrap}>
                                        <Reason icon="people" txt="3 passions communes" color={c.accentDeep} bg="rgba(255,140,190,0.14)" bd="rgba(255,140,190,0.25)" />
                                        <Reason icon="checkmark" txt="92% compatible" color="#1f9d57" bg="rgba(134,230,170,0.12)" bd="rgba(134,230,170,0.24)" />
                                        <Reason icon="flame" txt="Très actif" color="#e8893a" bg="rgba(255,157,107,0.12)" bd="rgba(255,157,107,0.24)" />
                                    </View>
                        ...
                        <View style={{ marginTop: 13, gap: 11 }}>
                            {nearShown.map((n, i) => {
                                ...
                                return (
                                    <FadeInUp key={`${n.id}-${i}`} delay={180 + i * 60}>
                                        <PressableScale style={s.nearCard} scaleTo={0.98} onPress={() => !n.full && router.push(`/(app)/cercles/${n.id}`)}>
                                            <View>
                                                <LinearGradient colors={n.grad as any} start={{ x: 0.15, y: 0 }} end={{ x: 0.85, y: 1 }} style={[s.nearTile, n.full && { opacity: 0.7 }]} />
                                                <View style={s.nearBadge}><GradeMini code={n.badge} /></View>
                                            </View>
                                            <View style={{ flex: 1 }}>
                                                <Text style={[s.nearName, n.full && { color: c.ink(0.6) }]}>{n.nom}</Text>
                                                <View style={s.nearMeta}>
                                                    <Text style={s.nearMetaTxt}>{n.info}</Text>
                                                    <Text style={s.nearDot}>·</Text>
                                                    <Text style={s.nearMetaTxt}>{n.dist}</Text>
                                                    {!!n.state && <Text style={s.nearDot}>·</Text>}
                                                    {!!n.state && (n.dot ? (
                                                        <View style={s.stateLive}><View style={s.liveDot} /><Text style={s.nearMetaTxt}>{n.state}</Text></View>
                                                    ) : <Text style={s.stateNew}>{n.state}</Text>)}
                                                </View>
                                            </View>
                                            {n.full ? (
                                                <View style={s.fullBtn}><Text style={s.fullBtnTxt}>Complet</Text></View>
                                            ) : isJoined ? (
                                                <View style={s.joinedBtn}><Ionicons name="checkmark" size={13} color="#1f9d57" /><Text style={s.joinedBtnTxt}>Rejoint</Text></View>
                                            ) : (
                                                <Pressable onPress={() => join(n.id)} style={s.joinBtn}><Text style={s.joinBtnTxt}>Rejoindre</Text></Pressable>
                                            )}
                                        </PressableScale>
                                    </FadeInUp>
                                );
                            ...
                            {nearShown.length === 0 && (
                                <View style={s.empty}>
                                    <Ionicons name="search" size={26} color={c.ink(0.25)} />
                                    <Text style={s.emptyTxt}>Aucun cercle ne correspond à ta recherche</Text>
                                </View>
                            )}
                        </View>
                    </View>
                    ...
                    <View>
                        <FadeInUp delay={120}>
                            <LinearGradient colors={['rgba(255,106,169,0.16)', 'rgba(255,111,194,0.08)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.limit}>
                                ...
                                <View style={s.limitBars}>
                                    ...
                                    <LinearGradient colors={gradients.rose} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.limitBar} />
                                    <View style={[s.limitBar, { backgroundColor: c.ink(0.12) }]} />
                                </View>
                        ...
                        {allMine.map((cc, i) => {
                            const g = GRADES[cc.grade] ?? GRADES.graine;
                            return (
                                <FadeInUp key={cc.id + i} delay={160 + i * 70}>
                                    <View style={s.myCard}>
                                        <View style={s.myTop}>
                                            <LinearGradient colors={cc.tile as any} start={{ x: 0.15, y: 0 }} end={{ x: 0.85, y: 1 }} style={s.myTile} />
                                            <View style={{ flex: 1 }}>
                                                <View style={s.myNameRow}>
                                                    <Text style={s.myName}>{cc.nom}</Text>
                                                ...
                                                <View style={s.myMeta}>
                                                    <View style={s.myMetaItem}><Ionicons name="person" size={11} color={c.ink(0.5)} /><Text style={s.myMetaTxt}>{cc.membres}</Text></View>
                                                    <Text style={s.nearDot}>·</Text>
                                                    <View style={s.modePill}><Text style={s.modePillTxt}>{cc.mode}</Text></View>
                                                </View>
                                            </View>
                                            <View style={s.unread}><Text style={s.unreadTxt}>{cc.unread}</Text></View>
                                        </View>
                                        <View style={{ paddingHorizontal: 14, paddingBottom: 12 }}>
                                            <View style={s.xpTop}>
                                                <Text style={s.xpLabel}>XP du cercle</Text>
                                                <Text style={s.xpVal}><Text style={s.xpValStrong}>{cc.xp}</Text> / {cc.xpMax}</Text>
                                            </View>
                                            <AnimatedBar progress={cc.xp / cc.xpMax} height={7} track={c.ink(0.12)}>
                                                <LinearGradient colors={gradients.rose} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ flex: 1 }} />
                                            </AnimatedBar>
                                        </View>
                                        <View style={s.myFoot}>
                                            <View style={s.myFootItem}><Ionicons name="time-outline" size={13} color={c.ink(0.55)} /><Text style={s.myFootTxt}>Jour {cc.jour} / 60</Text></View>
                                            <PressableScale onPress={() => router.push(`/(app)/cercles/${cc.id}`)}>
                                                <View style={s.openBtn}><Text style={s.openBtnTxt}>Ouvrir</Text><Ionicons name="chevron-forward" size={13} color={c.accentDeep} /></View>
                                            </PressableScale>
                                        </View>
                                    </View>
                                </FadeInUp>
                        ...
                        <FadeInUp delay={320}>
                            <Pressable style={s.createBtn} onPress={() => router.push('/(app)/creer')}>
...
function Seg({ label, count, active, onPress }: { label: string; count?: string; active?: boolean; onPress: () => void }) {
    ...
    const inner = (
        <>
            <Text style={[s.segTxt, { color: active ? '#fff' : c.ink(0.55) }]}>{label}</Text>
            {count && <Text style={[s.segCount, { color: active ? 'rgba(255,255,255,0.7)' : c.ink(0.4) }]}> {count}</Text>}
        </>
    );
    if (active) {
        return (
...
const makeStyles = (c: Palette) => StyleSheet.create({
    ...
    search: { flexDirection: 'row', alignItems: 'center', gap: 11, paddingVertical: 13, paddingHorizontal: 16, borderRadius: 16, backgroundColor: c.field, borderWidth: 1, borderColor: c.border },
    searchInput: { flex: 1, fontFamily: fonts.body, fontSize: 14, color: c.text, padding: 0 },
    joinedBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 9, paddingHorizontal: 14, borderRadius: 11, backgroundColor: 'rgba(123,224,160,0.16)', borderWidth: 1, borderColor: 'rgba(123,224,160,0.4)' },
    joinedBtnTxt: { fontFamily: fonts.bodyBold, fontSize: 12.5, color: '#1f9d57' },
    empty: { alignItems: 'center', gap: 10, paddingVertical: 36 },
    emptyTxt: { fontFamily: fonts.body, fontSize: 13.5, color: c.ink(0.45), textAlign: 'center' },

    segment: { marginTop: 16, flexDirection: 'row', padding: 5, borderRadius: 16, backgroundColor: c.field, borderWidth: 1, borderColor: c.border, gap: 0 },
    ...
    featured: { marginTop: 18, borderRadius: 24, overflow: 'hidden', borderWidth: 1, borderColor: c.border, backgroundColor: c.card, shadowColor: '#281950', shadowOffset: { width: 0, height: 18 }, shadowOpacity: c.mode === 'dark' ? 0.35 : 0.18, shadowRadius: 28, elevation: 4 },
    ...
    whyTxt: { fontFamily: fonts.bodyBold, fontSize: 13, color: c.accentDeep },
    ...
    av: { width: 28, height: 28, borderRadius: 14, borderWidth: 2, borderColor: c.card, marginLeft: -10 },
    avExtra: { backgroundColor: c.card2, alignItems: 'center', justifyContent: 'center' },
    avExtraTxt: { fontFamily: fonts.bodyBold, fontSize: 10, color: c.accent },
    membersCount: { fontFamily: fonts.bodyBold, fontSize: 13, color: c.text },
    membersBalance: { fontFamily: fonts.body, fontSize: 11, color: c.ink(0.45), marginTop: 1 },
    cta: { marginTop: 16, paddingVertical: 14, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, shadowColor: '#e02a73', shadowOffset: { width: 0, height: 12 }, shadowOpacity: 0.5, shadowRadius: 18, elevation: 5 },
    ...
    listHeadTitle: { fontFamily: fonts.display, fontSize: 16, color: c.text },
    seeAll: { flexDirection: 'row', alignItems: 'center', gap: 3 },
    seeAllTxt: { fontFamily: fonts.bodySemi, fontSize: 13, color: c.accent },

    nearCard: { flexDirection: 'row', alignItems: 'center', gap: 13, padding: 12, borderRadius: 18, backgroundColor: c.card, borderWidth: 1, borderColor: c.border },
    nearTile: { width: 58, height: 58, borderRadius: 15 },
    nearBadge: { position: 'absolute', top: -4, right: -4 },
    gradeMini: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: c.card, alignItems: 'center', justifyContent: 'center' },
    nearName: { fontFamily: fonts.bodyBold, fontSize: 14.5, color: c.text },
    nearMeta: { marginTop: 4, flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
    nearMetaTxt: { fontFamily: fonts.body, fontSize: 11.5, color: c.ink(0.5) },
    nearDot: { color: c.ink(0.3), fontSize: 11 },
    ...
    joinBtn: { paddingVertical: 9, paddingHorizontal: 15, borderRadius: 11, backgroundColor: 'rgba(255,140,190,0.16)', borderWidth: 1, borderColor: 'rgba(255,140,190,0.35)' },
    joinBtnTxt: { fontFamily: fonts.bodyBold, fontSize: 12.5, color: c.accentDeep },
    fullBtn: { paddingVertical: 9, paddingHorizontal: 15, borderRadius: 11, backgroundColor: c.field, borderWidth: 1, borderColor: c.border },
    fullBtnTxt: { fontFamily: fonts.bodyBold, fontSize: 12.5, color: c.ink(0.45) },

    limit: { marginTop: 18, padding: 15, borderRadius: 18, borderWidth: 1, borderColor: 'rgba(255,140,190,0.25)' },
    limitTop: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 },
    limitTitle: { fontFamily: fonts.bodyBold, fontSize: 13, color: c.text },
    limitFree: { fontFamily: fonts.body, fontSize: 11.5, color: c.ink(0.55) },
    ...
    limitNote: { fontFamily: fonts.body, fontSize: 11.5, color: c.ink(0.6), flex: 1 },
    limitBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 7, paddingHorizontal: 13, borderRadius: 10, shadowColor: '#e02a73', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.5, shadowRadius: 12, elevation: 4 },
    limitBtnTxt: { fontFamily: fonts.bodyBold, fontSize: 12, color: '#fff' },

    myCard: { marginTop: 14, borderRadius: 22, overflow: 'hidden', borderWidth: 1, borderColor: c.border, backgroundColor: c.card },
    ...
    myName: { fontFamily: fonts.display, fontSize: 16, color: c.text },
    ...
    myMetaTxt: { fontFamily: fonts.body, fontSize: 11.5, color: c.ink(0.5) },
    modePill: { paddingVertical: 2, paddingHorizontal: 7, borderRadius: 99, backgroundColor: 'rgba(255,140,190,0.16)' },
    modePillTxt: { fontFamily: fonts.bodySemi, fontSize: 11, color: c.accentDeep },
    unread: { minWidth: 24, height: 24, paddingHorizontal: 7, borderRadius: 12, backgroundColor: '#ff4d97', alignItems: 'center', justifyContent: 'center' },
    ...
    xpLabel: { fontFamily: fonts.body, fontSize: 11, color: c.ink(0.5) },
    xpVal: { fontFamily: fonts.body, fontSize: 11, color: c.ink(0.6) },
    xpValStrong: { fontFamily: fonts.bodyBold, color: c.text },
    myFoot: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 11, paddingHorizontal: 14, borderTopWidth: 1, borderTopColor: c.border, backgroundColor: c.ink(0.03) },
    myFootItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
    myFootTxt: { fontFamily: fonts.body, fontSize: 11.5, color: c.ink(0.55) },
    openBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 8, paddingHorizontal: 14, borderRadius: 10, backgroundColor: 'rgba(255,140,190,0.16)', borderWidth: 1, borderColor: 'rgba(255,140,190,0.3)' },
    openBtnTxt: { fontFamily: fonts.bodyBold, fontSize: 12.5, color: c.accentDeep },

    createBtn: { marginTop: 14, paddingVertical: 18, borderRadius: 20, backgroundColor: 'rgba(255,106,169,0.07)', borderWidth: 1.5, borderColor: 'rgba(255,140,190,0.45)', borderStyle: 'dashed', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 },
    createBtnTxt: { fontFamily: fonts.bodyBold, fontSize: 14, color: c.accent },
});
